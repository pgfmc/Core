...
public class Request {
	...
	public interface RequestMessage
	{
		...
		public RequestMessage def = (initiate, target, r, req) -> {
			switch (r) {
			case Accept:
				initiate.sendMessage("§aAccepted " + req.parent.getName() + " request -> " + target.getRankedName());
				target.sendMessage("§aAccepted " + req.parent.getName() + " request <- " + initiate.getRankedName());
				break;
			
			case Deny:
				initiate.sendMessage("§cDenied " + req.parent.getName() + " request -> " + target.getRankedName());
				target.sendMessage("§cDenied " + req.parent.getName() + " request <- " + initiate.getRankedName());
				break;
			...
			case Quit:
				try {
					initiate.sendMessage("§cQuit " + req.parent.getName() + " request -> " + target.getRankedName()); } finally {}
				try {
					target.sendMessage("§cQuit " + req.parent.getName() + " request <- " + initiate.getRankedName()); } finally {}
				break;
			
			case Timeout:
				try {
					initiate.sendMessage("§cTimeout " + req.parent.getName() + " request -> " + target.getRankedName()); } finally {}
				try {
					target.sendMessage("§cTimeout " + req.parent.getName() + " request <- " + initiate.getRankedName()); } finally {}
				break;
			
			case Force:
				try {
					initiate.sendMessage("§cStopped " + req.parent.getName() + " request -> " + target.getRankedName()); } finally {}
				try {
					target.sendMessage("§cStopped " + req.parent.getName() + " request <- " + initiate.getRankedName()); } finally {}
				break;
			}
		};
	}
	...
	protected Request(PlayerData initiate, PlayerData target, Requester parent) {
		...
		{
			initiate.sendMessage("§6Outgoing " + parent.getName() + " request -> " + target.getRankedName());
			target.sendMessage("§6Incoming " + parent.getName() + " request <- " + initiate.getRankedName());
		}
